...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 2. Calculate RMS: sqrt(mean(x^2) + eps)
        # We compute the mean across the last dimension (d_model)
        # keepdim=True ensures shape allows broadcasting later: (Batch, Seq, 1)
        mean_square = einops.reduce(x.pow(2), '... d_model -> ... 1', 'mean')
        rms = torch.sqrt(mean_square + self.eps)

        # 3. Normalize
        x_norm = x_fp32 / rms

        # 4. Scale with learnable parameter 'g' (self.weight)
        # self.weight broadcast from (d_model,) to (Batch, Seq, d_model)
        output = einops.einsum(x_norm, self.weight, "... d, d -> ... d")
