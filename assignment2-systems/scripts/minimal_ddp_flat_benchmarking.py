...
def allreduce_grads_flattened(model:nn.Module, world_size: int):
    # 1) 收集所有 grad tensor（只收 dense 的、非 None 的）
    grads = [p.grad for p in model.parameters() if p.grad is not None]

    # 2) flatten：把多个张量拼成一个 1-D 大张量
    flat = torch._utils._flatten_dense_tensors(grads)  # shape: [sum(numel)]

    # 3) 一次 all-reduce
    dist.all_reduce(flat, op=dist.ReduceOp.SUM)
    flat.div_(world_size)

    # 4) unflatten：把 flat 按原来每个 grad 的形状切回去
    new_grads = torch._utils._unflatten_dense_tensors(flat, grads)

    # 5) 写回每个参数的 grad（原地拷贝，避免改引用）
    i = 0
    for p in model.parameters():
        if p.grad is None:
            continue
        p.grad.copy_(new_grads[i])
        i += 1
...
def worker(rank: int, world_size: int, args, global_inputs, global_targets):
    ...
    for _ in range(args.warmup_steps):
        ...
        loss.backward()
        allreduce_grads_flattened(model, world_size)
        optimizer.step()
    ...
    for i in range(args.steps):
        ...
        # Start Total Step Timer
        start_event.record()

        # A. Computation (Forward + Backward)
        logits = model(x)
        loss = cross_entropy_loss(logits.view(-1, config['vocab_size']), y.view(-1))
        loss.backward()

        # B. Communication (Naive All-Reduce)
        comm_start.record()
        allreduce_grads_flattened(model, world_size)
        comm_end.record()
