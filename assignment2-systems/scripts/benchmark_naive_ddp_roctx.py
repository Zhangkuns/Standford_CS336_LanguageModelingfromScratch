...
from cs336_systems.roctx import roctx_range, roctx_mark
...
def worker(rank: int, world_size: int, args, global_inputs, global_targets):
    ...
    with roctx_range("Benchmark Loop"):
        for i in range(args.steps):
            # Annotate individual steps
            with roctx_range(f"Step {i}"):
                optimizer.zero_grad()
                ...
                with roctx_range("Backward Pass"):
                    loss.backward()
                ...
                # C. Update
                with roctx_range("Optimizer Step"):
                    optimizer.step()
